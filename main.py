...
@app.get(
    ...
    description="Devuelve todos los dispositivos IoT registrados en el sistema, con su estado activo/inactivo."
)
...
@app.post(
    ...
    description="Recibe eCO2, TVOC, temperatura y humedad de un dispositivo, calcula el nivel de riesgo, y genera una alerta por Telegram si el riesgo es alto."
)
...
@app.get(
    ...
    description="Devuelve eCO2, TVOC, temperatura, humedad y nivel de riesgo de la medición más reciente."
)
def obtener_ultima_medicion(db: Session = Depends(get_db)):
    ...
    for valor in valores:
        tipo = (
            ...
            .filter(models.TipoMedicion.id == valor.tipo_medicion_id)
            .first()
        )

        if tipo:
            datos[tipo.nombre.lower()] = valor.valor
...
@app.get(
    ...
    "USAR ÚNICAMENTE cuando el usuario solicite las últimas N mediciones "
    "o necesite consultar valores individuales de mediciones recientes. "
    "El parámetro 'limit' es obligatorio y debe ser un número entero "
    "entre 1 y 100. Nunca enviar limit=null."
)
...
@app.get(
    ...
    description=(
    "Obtiene TODAS las mediciones que VIPIS ha clasificado con nivel "
    "de riesgo 'Crítico'. USAR cuando el usuario solicite identificar, "
    "listar o consultar las mediciones críticas históricas. "
    "No utilizar esta herramienta para calcular promedios, máximos, "
    "mínimos u otras estadísticas generales."
)
...
def obtener_mediciones_criticas(
    ...
    mediciones = (
        ...
        .join(
            models.NivelRiesgo,
            models.NivelRiesgo.id == models.Clasificacion.nivel_riesgo_id
        )
        .filter(
...
@app.get(
    ...
    "\n\n"
    "Esta herramienta devuelve directamente los cálculos realizados "
    "por la API y NO requiere parámetros. "
    "\n\n"
    "IMPORTANTE: el campo 'mediciones_criticas' contiene directamente "
    "la cantidad TOTAL de mediciones clasificadas como 'Crítico'. "
    "NO es necesario consultar /mediciones/criticas para obtener "
    "esta cantidad. "
    "\n\n"
    "IMPORTANTE: el campo 'promedio_eco2_todas_las_mediciones' "
    "contiene directamente el promedio de eCO2 de TODAS las "
...
@app.get(
    ...
    description="Devuelve todas las alertas de calidad del aire que aún no han sido atendidas, con su nivel de riesgo y valores de medición."
)
...
@app.put(
    ...
    description="Cambia el estado de una alerta específica a 'atendida' usando su ID."
)
def atender_alerta(
    alerta_id: int,
    db: Session = Depends(get_db)
...
@app.get(
    ...
    description="Devuelve los rangos de valores y niveles de riesgo (Excelente, Bueno, ..., Crítico) configurados por tipo de medición."
)
def listar_rangos(
    db: Session = Depends(get_db)
):
